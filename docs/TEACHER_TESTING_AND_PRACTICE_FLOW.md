...
- The lesson stores its own quiz (`Lesson.quiz.questions`). Practice uses attached questions and/or topic-scoped banks.
...
This document describes the **current system**: lesson quiz from the lesson’s quiz, practice from topic-scoped banks (and attached questions), and access controlled by the backend (e.g. `accessDecision.allowed`). Use it as the single source of truth for teacher onboarding, support, and product/engineering alignment.
