 ...
 * GET /api/autopilot0/execution-contract-intelligence — admin-only execution contract observer (read-only).
 */
...
router.get("/execution-contract-intelligence", auth, requireAdmin, async (req, res) => {
  try {
    if (req.query.tier !== undefined) {
      return res.status(400).json({
        error: "tier is not supported in execution-contract-intelligence V1; cohort scope is SPEC_ONLY",
      });
    }
    ...
    const report = await buildExecutionContractIntelligence({ specKey, limit });
    return res.json(report);
  } catch (err) {
    if (err.code === "INVALID_SPEC_KEY") {
      return res.status(400).json({ error: err.message });
    }
    return sendInternalError("autopilot0/execution-contract-intelligence", err, res);
  }
});
