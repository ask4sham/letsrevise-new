...
/**
 * Aggregate counts by topicKey for a model. Uses $or for ownerId/teacherId so both schemas work.
 */
...
async function aggregateCounts(Model, userId, candidateKeys, extraMatch = {}) {
  ...
  const match = {
    $and: [
      ...
      { $or: [{ teacherId: userOid }, { ownerId: userOid }] },
...
router.get("/", auth, async (req, res) => {
  ...
  try {
    ...
    const [flashMap, quizMcqMap, quizShortMap, examMap, pastPaperQMap] = await Promise.all([
      aggregateCounts(TopicFlashcard, userId, candidateKeysAll),
      aggregateCounts(TopicQuizQuestion, userId, candidateKeysAll, { type: "mcq" }),
      aggregateCounts(TopicQuizQuestion, userId, candidateKeysAll, { type: "short-answer" }),
      aggregateCounts(ExamQuestion, userId, candidateKeysAll),
      aggregateCounts(PastPaperQuestion, userId, candidateKeysAll),
    ]);
    ...
    const unitsOut = units.map((unit) => {
      const topicsOut = (unit.topics || []).map((t) => {
        ...
        const sumFrom = (m) => candidates.reduce((acc, k) => acc + (m.get(k) || 0), 0);

        const counts = {
          flashcards: sumFrom(flashMap),
          quiz_mcq: sumFrom(quizMcqMap),
          quiz_short: sumFrom(quizShortMap),
          examQuestions: sumFrom(examMap),
          pastPaperQuestions: sumFrom(pastPaperQMap),
        };
    ...
    return res.json({ specKey, units: unitsOut, totals });
  } catch (err) {
