...
function isClassroomPresentRequest(req) {

  return req.query.present === "classroom";
...
module.exports = function canAccessContent(options = {}) {
  ...
  return async function (req, res, next) {
    ...
    try {
      ...
      const classroomMode = isClassroomPresentRequest(req);
      ...
      const sharedAccess =
        ...
        !isOwner && isTeacherUser && userId != null

          ? await resolveTeacherShareAccess(lessonId, userId, lesson, { classroomMode })
      ...
      if (requirePublished && !isPublished) {
        ...
        if (sharedAccess) {
          ...
          req.accessDecision = { allowed: true, reason: sharedAccess.reason };
        ...
        if (isTeacherUser && !isOwner) {
          ...
          if (classroomMode) {
            ...
            return res.status(403).json(denyBody(lessonIdStr, "CLASSROOM_TEACH_REQUIRED", "Teaching access required for classroom mode", isPublished));
