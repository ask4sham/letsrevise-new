/**
 * Rate limit for lesson revision pack PDF export.
 * Students 2/min, teachers 6/min, admins 20/min.
 */
const rateLimit = require("express-rate-limit");

const lessonRevisionPackExportRateLimit = rateLimit({
  windowMs: 60 * 1000,
  max: (req) => {
  ...
  legacyHeaders: false,
  keyGenerator: (req) => `lessonRevisionPackExport:${req.user?._id || req.user?.userId || req.ip}`,
  skip: (req) => {
    const role = (req.user?.userType || req.user?.role || "").toString().toLowerCase();
...
module.exports = lessonRevisionPackExportRateLimit;
