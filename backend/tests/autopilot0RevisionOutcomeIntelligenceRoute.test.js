/**
 * Autopilot 0 Revision Outcome Intelligence route — admin-only GET /api/autopilot0/revision-outcome-intelligence.
 */
...
describe("GET /api/autopilot0/revision-outcome-intelligence", () => {
  const app = buildApp();
  let adminToken;
  let teacherToken;
  ...
  beforeAll(async () => {
    const ts = Date.now();
    adminToken = await loginAs(app, { email: `ap0ro-admin-${ts}@test.com`, userType: "admin" });
    teacherToken = await loginAs(app, { email: `ap0ro-teacher-${ts}@test.com`, userType: "teacher" });
  });
  ...
  test("admin 200 with expected version and level", async () => {
    const res = await request(app)
      .get("/api/autopilot0/revision-outcome-intelligence")
      .query({ specKey: "aqa-gcse-biology" })
      .set("Authorization", `Bearer ${adminToken}`);
    expect(res.status).toBe(200);
    expect(res.body.version).toBe("autopilot0-revision-outcome-intelligence-v1");
    ...
    expect(res.body.topicOutcomes).toEqual([]);
    expect(buildRevisionOutcomeIntelligence).toHaveBeenCalledWith({
      specKey: "aqa-gcse-biology",
      limit: 20,
    });
  });
  ...
  test("invalid specKey rejected", async () => {
    buildRevisionOutcomeIntelligence.mockRejectedValueOnce(
  ...
  test("tier rejected", async () => {
    ...
    expect(buildRevisionOutcomeIntelligence).not.toHaveBeenCalled();
  });
  ...
  test("route file wires auth and requireAdmin", () => {
    ...
    expect(routeSource).toMatch(/revision-outcome-intelligence[\s\S]*auth,\s*requireAdmin/);
