/**
 * Autopilot 0 Learning Trend Intelligence V1 — unit tests.
 */
const fs = require("fs");
...
const SERVICE_PATH = path.join(
  ...
  "services",
  "autopilot0",
  "learningTrendIntelligenceService.js"
);
...
describe("learningTrendIntelligenceService contract", () => {
  test("version and level", () => {
    expect(VERSION).toBe("autopilot0-learning-trend-intelligence-v1");
...
describe("window cutoffs", () => {
  ...
  test("earlier/recent boundary handling", () => {
    ...
    expect(
      classifyEventWindow(new Date(cutoffs.earlierStart.getTime() - 1), cutoffs)
    ).toBeNull();
  });
});
...
describe("canonical alias merge before mastery", () => {
  test("merges alias topic keys into one student canonical window", () => {
    ...
    expect(merged[0].quizCorrect).toBe(3);
...
describe("mastery parity", () => {
  ...
  test("exam-only mastery matches A0.3", () => {
    expect(masteryFromCountRow({ quizAttempts: 0, quizCorrect: 0, examAttempts: 5, examCorrect: 4 })).toBe(
      computeMasteryFromCounts(0, 0, 5, 4)
    );
  });
...
describe("median and trend thresholds", () => {
  test("odd median", () => {
    expect(medianInteger([-5, 0, 10])).toBe(0);
  });

  test("even median rounds average of middle pair", () => {
    expect(medianInteger([-10, 0, 4, 10])).toBe(2);
  });
...
describe("paired cohort gates", () => {
  test("unpaired students excluded from topic trend", () => {
    ...
    const result = computeTopicTrend(CANONICAL, topicData);
  ...
  test("10 paired students eligible with sufficient attempts", () => {
    const topicData = topicDataFromRows(
...
describe("ordering", () => {
  test("deterministic trend priority and limit after sort", () => {
    const sorted = sortTopicTrends([
      { topicKey: `${SPEC}:z-topic`, trend: "STABLE", medianDeltaPercentagePoints: 0 },
      { topicKey: `${SPEC}:a-topic`, trend: "IMPROVING", medianDeltaPercentagePoints: 20 },
      { topicKey: `${SPEC}:b-topic`, trend: "DECLINING", medianDeltaPercentagePoints: -20 },
      { topicKey: `${SPEC}:c-topic`, trend: "DECLINING", medianDeltaPercentagePoints: -10 },
    ]);
    expect(sorted.map((r) => r.trend)).toEqual([
      "DECLINING",
      "DECLINING",
      "IMPROVING",
      "STABLE",
    ...
    expect(sorted[2].medianDeltaPercentagePoints).toBe(20);
    expect(sorted.slice(0, 2).map((r) => r.trend)).toEqual(["DECLINING", "DECLINING"]);
  });
});

describe("buildLearningTrendIntelligence integration", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    adminTaxonomyService.getMergedTaxonomyBySpecKey.mockResolvedValue(TAXONOMY);
  });

  test("declining topic → AMBER", async () => {
    LearningEvidenceEvent.aggregate.mockResolvedValue(
      pairedStudentRows({ studentCount: 10, earlierMastery: 80, recentMastery: 50, attemptsPerWindow: 5 })
    );
    const report = await buildLearningTrendIntelligence({ specKey: SPEC, now: FIXED_NOW });
    expect(report.summary.overallStatus).toBe("AMBER");
    expect(report.summary.humanReviewRequired).toBe(true);
    expect(report.topicTrends[0].trend).toBe("DECLINING");
    expect(assertValidSpecKey).toHaveBeenCalledWith(SPEC);
  });

  test("improving-only → GREEN", async () => {
    LearningEvidenceEvent.aggregate.mockResolvedValue(
      pairedStudentRows({ studentCount: 10, earlierMastery: 40, recentMastery: 70, attemptsPerWindow: 3 })
    );
    const report = await buildLearningTrendIntelligence({ specKey: SPEC, now: FIXED_NOW });
    expect(report.summary.overallStatus).toBe("GREEN");
    expect(report.summary.humanReviewRequired).toBe(false);
    expect(report.topicTrends[0].trend).toBe("IMPROVING");
  });

  test("stable-only → GREEN", async () => {
    LearningEvidenceEvent.aggregate.mockResolvedValue(
      pairedStudentRows({ studentCount: 10, earlierMastery: 70, recentMastery: 72, attemptsPerWindow: 3 })
    );
    const report = await buildLearningTrendIntelligence({ specKey: SPEC, now: FIXED_NOW });
    expect(report.summary.overallStatus).toBe("GREEN");
    expect(report.topicTrends[0].trend).toBe("STABLE");
  });

  test("no eligible topics → UNKNOWN", async () => {
    LearningEvidenceEvent.aggregate.mockResolvedValue([]);
    const report = await buildLearningTrendIntelligence({ specKey: SPEC, now: FIXED_NOW });
    expect(report.summary.overallStatus).toBe("UNKNOWN");
    expect(report.summary.humanReviewRequired).toBe(true);
    expect(report.eligibleTopicCount).toBe(0);
  });

  test("response has no student identifiers", async () => {
    LearningEvidenceEvent.aggregate.mockResolvedValue(
      pairedStudentRows({ studentCount: 10, attemptsPerWindow: 3 })
    );
    const report = await buildLearningTrendIntelligence({ specKey: SPEC, now: FIXED_NOW });
  ...
  test("limit applied after sorting", async () => {
    ...
    const report = await buildLearningTrendIntelligence({
      specKey: SPEC,
      now: FIXED_NOW,
      limit: 1,
    });
    expect(report.topicTrends).toHaveLength(1);
    expect(report.topicTrends[0].trend).toBe("DECLINING");
  });

  test("aggregate match requires boolean correct and eligible event types", async () => {
    LearningEvidenceEvent.aggregate.mockResolvedValue([]);
    await buildLearningTrendIntelligence({ specKey: SPEC, now: FIXED_NOW });
