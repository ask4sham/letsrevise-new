/**
 * Autopilot 0 Safe Action Readiness Observer route — admin-only GET /api/autopilot0/action-readiness-intelligence.
 */
...
describe("GET /api/autopilot0/action-readiness-intelligence", () => {
  const app = buildApp();
  let adminToken;
  let teacherToken;
  ...
  beforeAll(async () => {
    const ts = Date.now();
    adminToken = await loginAs(app, { email: `ap0ari-admin-${ts}@test.com`, userType: "admin" });
    teacherToken = await loginAs(app, { email: `ap0ari-teacher-${ts}@test.com`, userType: "teacher" });
  });
  ...
  test("admin 200 with expected version and level", async () => {
    const res = await request(app)
      .get("/api/autopilot0/action-readiness-intelligence")
      .query({ specKey: "aqa-gcse-biology" })
      .set("Authorization", `Bearer ${adminToken}`);
    expect(res.status).toBe(200);
    expect(res.body.version).toBe("autopilot0-action-readiness-intelligence-v1");
    expect(res.body.level).toBe("L0");
    expect(res.body.cohort.cohortScope).toBe("SPEC_ONLY");
    expect(res.body.topicReadiness).toEqual([]);
    expect(res.body.policy.l1ExecutionEnabled).toBe(false);
    expect(res.body.summary.l1EligibleCount).toBe(0);
  ...
  test("invalid specKey rejected", async () => {
    buildActionReadinessIntelligence.mockRejectedValueOnce(
  ...
  test("tier rejected", async () => {
    ...
    expect(buildActionReadinessIntelligence).not.toHaveBeenCalled();
  });
  ...
  test("route file wires auth and requireAdmin", () => {
    ...
    expect(routeSource).toMatch(/action-readiness-intelligence[\s\S]*auth,\s*requireAdmin/);
