...
function ensureExactlyTenKeywords(text, keywordContext, fixes) {
  ...
  if (kwIx < 0) {
    const rows = resolveKeywordRows(keywordContext, 10, text, fixes);
    const block = [
      "0 — KEY WORDS",
  ...
  if (rows.length < 10) {
    rows = padKeywordRows(rows, keywordContext, text, 10, fixes);
  }
...
function templateKeywordsDefault(topicLabel, keywordContext, lessonText) {
  const rows = resolveKeywordRows(keywordContext, 10, lessonText, null);
  const lines = rows
...
function isThinBodyForSlot(body, slotIndex) {
  ...
  if (key === "keywords") return plainLen < 120;
...
export function deterministicAutoFixLesson({
  ...
  const examPracticeGate = evaluateExamPracticeAuthorityGate(working, {
    ...
    topicKey,
    subTopic: topicLabel,
    subTopicProfile: examFix.profile || scopeFix.profile,
  });
