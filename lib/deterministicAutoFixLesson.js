...
const { applyQuestionDeduplicationGuard } = require("./questionDeduplicationGuard.js");
const {
...
const {
  ...
  rpSlotBodyHtml,
} = require("./teacherBrain/requiredPracticalMode.js");
...
function rpContextFromKeyword(keywordContext = {}) {
  return {
    topic: keywordContext.topic,
    topicKey: keywordContext.topicKey,
    subTopic: keywordContext.subTopic || keywordContext.topic,
  };
}

function resolveSs1CanonicalSlots(keywordContext = {}) {
  if (isRequiredPracticalMode(rpContextFromKeyword(keywordContext))) {
    return REQUIRED_PRACTICAL_SS1_CANONICAL_SLOTS;
  }
  return isTeacherFirstSs1Enabled() ? TEACHER_FIRST_SS1_SHELL_SLOTS : CLASSIC_SS1_SHELL_SLOTS;
}

function getCoreTeachingOverflowSlotIndex(keywordContext = {}) {
  const slots = resolveSs1CanonicalSlots(keywordContext);
  const overflowKey = isRequiredPracticalMode(rpContextFromKeyword(keywordContext))
    ? "method"
    : "coreTeaching";
  return slots.findIndex((s) => s.key === overflowKey);
}

function slotKeyForIndex(index, keywordContext = {}) {
  return resolveSs1CanonicalSlots(keywordContext)[index]?.key || null;
}

/** Map an SS1 block header / paste line to a canonical slot key (position-independent). */
function slotKeyFromBlockHeader(blockText = "", keywordContext = {}) {
...
function indexModelBlockBodiesBySlotKey(modelBlocks = [], keywordContext = {}) {
  const byKey = {};
  for (const blockText of modelBlocks) {
    const key = slotKeyFromBlockHeader(blockText, keywordContext);
    if (!key) continue;
...
function checkpointBody(topicLabel, preset = "main") {
  ...
  if (preset === "quick") {
    return [
      "Question:",
      `Explain one reason why ${t} matters in living organisms.`,
      "",
      "Option 1:",
      "Only a keyword with no mechanism",
      "",
      "Option 2:",
      "A clear mechanism linked to an outcome",
      "",
      "Option 3:",
      "Only a diagram with no words",
      "",
      "Option 4:",
  ...
  return [
    "Question:",
    `Which idea best explains a key step in ${t}?`,
    "",
    "Option 1:",
...
const OBJECTIVES_BLOCK = (topic, keywordContext = {}) => {
  ...
  if (isRequiredPracticalMode(rpContextFromKeyword(keywordContext))) {
    return [
      ...
      "",
      "<ul>",
      `<li><strong>👉</strong> State the <strong>aim</strong> of the <strong>${t}</strong> investigation.</li>`,
      "<li><strong>👉</strong> Identify <strong>independent, dependent and control variables</strong>.</li>",
      "<li><strong>👉</strong> Describe a <strong>safe, repeatable method</strong> and process results.</li>",
...
function ensureObjectives(text, topic, fixes, keywordContext = {}) {
  let t = norm(text);
  ...
  const tmpl = OBJECTIVES_BLOCK(topic, keywordContext);
...
function templateDefinitionDefault(topic) {
  const t = escapeHtmlInline(sanitizeTopic(topic));
  return [`<p>👉 <strong>[Clear GCSE definition of ${t} — no story]</strong></p>`].join("\n");
}

function templateWhyItMattersDefault(topic) {
  return ["<p>👉 <strong>[One sentence on why this concept matters in biology or exams]</strong></p>"].join(
    "\n"
  );
}
...
function templateCoreModelDefault(topic) {
  const t = escapeHtmlInline(sanitizeTopic(topic));
  return [`<p>👉 <strong>[The key GCSE model or pathway for ${t}]</strong></p>`].join("\n");
}
...
function templateWorkedExampleDefault(topic) {
  ...
  return [
    "Question:",
    `Explain how ${sanitizeTopic(topic)} works using a cause → effect chain.`,
    "",
    "Answer:",
    "<details>",
    "<summary>Reveal Answer</summary>",
...
function templateSelfCheckDefault(topic) {
  const t = escapeHtmlInline(sanitizeTopic(topic));
  return [
    `<p><strong>Self-check:</strong> Explain the role of one key structure or term in <strong>${t}</strong>, using because → therefore.</p>`,
    "<details>",
    "<summary>Reveal Answer</summary>",
    "",
    "<p>Name the structure/term, say <em>because</em>, and link to a measurable effect for this topic.</p>",
    "",
    "</details>",
  ].join("\n");
}
...
function defaultBodyForSlot(i, topicLabel, keywordContext, lessonText) {
  const key = slotKeyForIndex(i, keywordContext);
  if (isRequiredPracticalMode(rpContextFromKeyword(keywordContext))) {
    const plan = buildRequiredPracticalOpeningPlan(rpContextFromKeyword(keywordContext));
    const rpBody = rpSlotBodyHtml(key, plan);
    if (rpBody) return rpBody;
  }
  switch (key) {
    case "objectives":
...
function isThinBodyForSlot(body, slotIndex, keywordContext = {}) {
  const b = String(body || "").trim();
  if (!b) return true;
  const key = slotKeyForIndex(slotIndex, keywordContext);
...
function normalizeToCanonicalSs1Shell(text, topicLabel, fixes, keywordContext) {
  const canonicalSlots = resolveSs1CanonicalSlots(keywordContext);
  const coreTeachingOverflowIndex = getCoreTeachingOverflowSlotIndex(keywordContext);
  const chunks = lessonChunks(text);
  const prelude = [];
  const modelBlocks = [];
  ...
  let mergedOverflow = "";
  const bodiesBySlotKey = indexModelBlockBodiesBySlotKey(modelBlocks, keywordContext);
  if (modelBlocks.length > canonicalSlots.length) {
    structuralChange = true;
    mergedOverflow = modelBlocks
      .slice(canonicalSlots.length)
      .filter((blockText) => !slotKeyFromBlockHeader(blockText, keywordContext))
  ...
  for (let i = 0; i < canonicalSlots.length; i++) {
    ...
    if (!body && modelBlocks[i]) {
      const positionalKey = slotKeyFromBlockHeader(modelBlocks[i], keywordContext);
    ...
    if (isThinBodyForSlot(body, i, keywordContext)) {
...
export function deterministicAutoFixLesson({
  ...
  const rpMode = isRequiredPracticalMode({
    topic: topicLabel,
    topicKey,
    subTopic: topic,
  });
  ...
  working = ensureObjectives(working, topicLabel, fixesApplied, keywordContext);
  ...
  let checkpointFix = { profile: null };
  if (!rpMode) {
    checkpointFix = ensureCheckpointScopeCompliance(
      working,
      { topic: topicLabel, topicKey, subTopic: topicLabel },
      fixesApplied
    );
  ...
  let memoryRuleFix = { profile: null };
  if (!rpMode) {
    memoryRuleFix = ensureMemoryRuleScopeCompliance(
      working,
      { topic: topicLabel, topicKey, subTopic: topicLabel },
      fixesApplied
    );
  ...
  if (!rpMode) {
    const dedupe = applyQuestionDeduplicationGuard(working, {
      topic: topicLabel,
      topicKey,
      subTopic: topic,
    });
    if (dedupe.changed) {
      working = dedupe.text;
      fixesApplied.push(...dedupe.fixes);
    }
  }
