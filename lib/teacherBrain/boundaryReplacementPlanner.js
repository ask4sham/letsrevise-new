...
function formatBoundaryReplacementAppendix(replacementPlan) {
  if (!replacementPlan?.promptInstructions?.length) return "";
  const lines = [
    REPLACEMENT_MARKER,
    "",
    ...replacementPlan.promptInstructions,
  ];
  if (replacementPlan.rerouteActive) {
    lines.push("", "Reroute enforcement is ON for this generation request.");
  }
  return lines.join("\n");
}
...
function boundaryReplacementResponseMeta(plan) {
  if (!plan?.replacementPlans?.length && !plan?.blockedConceptIds?.length) {
    return null;
  }
  return {
