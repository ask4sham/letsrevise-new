...
function pickReplacementTarget(targetIds, coverageMap, profile) {
  ...
  const priorityProfile = require("./conceptPriorityEngine").resolveConceptPriorityProfile({
    subTopicProfile: profile,
  });
  const priorityPick = pickHighestPriorityConceptId(ids, priorityProfile, coverageMap);
...
function planBoundaryReplacements(input = {}) {
  ...
  let preferredConceptIds = [
    ...new Set([...preferredFromPlans, ...underTestedPrimaries, profile.centralConceptId].filter(Boolean)),
  ];
  const priorityProfile = require("./conceptPriorityEngine").resolveConceptPriorityProfile({
    subTopicProfile: profile,
  });
  if (priorityProfile && require("./conceptPriorityEngine").isPriorityEngineEnabled()) {
    const rows = coverageMap?.concepts || [];
    const byId = new Map(rows.map((c) => [c.id, c]));
    preferredConceptIds = require("./conceptPriorityEngine")
      .sortConceptsByPriority(
        preferredConceptIds.map((id) => byId.get(id) || { id, taughtCount: 0, testedCount: 0 }),
        priorityProfile
      )
      .map((c) => c.id);
  }
