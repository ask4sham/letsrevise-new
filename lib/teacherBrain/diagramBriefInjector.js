...
} = require("./dragDropVisualContract");
...
const { isTopicKindBlockedForProfile } = require("./interactionAuthorityEnforcer");
const {
  getInteractiveDiagramTopicSpec,
  formatInteractiveDiagramTopicBrief,
} = require("./interactiveDiagramTopicSpecs");

/** Temporary diagnostics for interactive diagram topic specialization. */
function logInteractiveDiagramBriefResolution(payload) {
  console.log("[TeacherBrainInteractiveDiagram]", payload);
}

/** Temporary diagnostics for interactive sequence topic specialization. */
function logInteractiveSequenceBriefResolution(payload) {
  console.log("[TeacherBrainInteractiveSequence]", payload);
}

const {
  resolveInteractiveSequenceTopicKind,
} = require("./interactiveSequenceTopicKind");
const {
  getInteractiveSequenceTopicSpec,
  formatInteractiveSequenceTopicBrief,
} = require("./interactiveSequenceTopicSpecs");
const {
...
function getBlockPairs(block) {
  ...
  return pairsFromBlock(block).map((p, i) => ({
    ...p,
    id: p.id || `pair-${i + 1}`,
    imageUrl: safeStr(rawPairs[i]?.imageUrl ?? rawPairs[i]?.image_url),
  }));
}
...
function deriveCardsAndMatches(block, diagram, briefData) {
  if (briefData?.suggestedCards?.length) {
    const cards = briefData.suggestedCards.map((c) => c.prompt).filter(Boolean);
    const matches = briefData.suggestedCards.map((c) => c.answer).filter(Boolean);
    const correctMatches = briefData.suggestedCards
  ...
  if (pairs.length && !pairsAreGeneric(pairs)) {
...
function deriveCommonMisconceptions(briefData) {
  if (briefData?.commonMisconceptions?.length) {
    return briefData.commonMisconceptions;
  }
  return [];
}

function buildBriefGenerationInput(block, ctx, diagram) {
  return {
    lesson: {
      topic: ctx.topic,
      topicKey: ctx.topicKey,
      subTopic: ctx.subTopic,
      title: safeStr(ctx.lessonTitle),
    },
    pages: ctx.pages || [],
    page: ctx.pages?.[ctx.pageIndex],
 ...
 * @param {object} [block]
 */
function formatTextMatchBrief(activity, diagram, block = {}, briefData = null) {
  const generatedNote =
    briefData?.contentDerived && briefData.suggestedCards?.length
      ? formatTeacherBrainBriefNote(briefData, { layout: "textMatch" })
      : null;
  if (generatedNote) return generatedNote;

  const { cards, matches, correctMatches } = deriveCardsAndMatches(block, diagram, briefData);
  ...
  return [
    ...
    deriveCommonMisconceptions(briefData).length
      ? dotList(deriveCommonMisconceptions(briefData))
    ...
    "",
    "Student task:",
    "Drag each card to its correct match using vocabulary from the lesson.",
    "",
...
function formatTextToImageBrief(activity, diagram, block = {}) {
  ...
  return [
    ...
    dotList(assessment),
    "",
    formatTextToImageImageDesignRequirements({ pairs: getBlockPairs(block) }),
    "",
...
function formatImageDropZonesBrief(activity, diagram, block = {}) {
  ...
  return [
    ...
    dotList(assessment),
    "",
    formatDragDropImageDesignRequirements({ pairs }),
    "",
 ...
 * @param {object} [activity]
 */
function formatStepByStepBrief(diagram, activity, briefData = null) {
  const generatedNote =
    briefData?.contentDerived && briefData.suggestedSteps?.length
      ? formatTeacherBrainBriefNote(briefData, { layout: "stepByStep" })
      : null;
  if (generatedNote) return generatedNote;
...
function buildBriefForBlock(block, ctx) {
  ...
  const briefData = generateTeacherBrainBrief(
    buildBriefGenerationInput(block, ctx, diagramPick.diagram)
  );
  ...
  if (typeNorm === "dragdropmatch") {
    ...
    return formatTextMatchBrief(activity, diagramPick.diagram, block, briefData);
  }
  if (typeNorm === "interactivesequence") {
    ...
    let sequenceTopicKind = resolveInteractiveSequenceTopicKind({
      topic: ctx.topic,
      topicKey: ctx.topicKey,
      subTopic: ctx.subTopic,
    });
    if (isTopicKindBlockedForProfile(sequenceTopicKind, profile)) {
      sequenceTopicKind = "generic";
    }
    const sequenceSpec = getInteractiveSequenceTopicSpec(sequenceTopicKind);
    const sequenceTemplate =
      sequenceSpec?.briefHeader || "STEP-BY-STEP BRIEF (generic/metabolism)";
    logInteractiveSequenceBriefResolution({
      topic: ctx.topic,
      topicKey: ctx.topicKey,
      subTopic: ctx.subTopic,
      resolvedKind: sequenceTopicKind,
      selectedTemplate: sequenceTemplate,
      blockType: typeNorm,
    });
    if (sequenceSpec) {
      return formatInteractiveSequenceTopicBrief(sequenceSpec);
    }
    ...
    return formatStepByStepBrief(stepDiagram, activity, briefData);
  }
  if (typeNorm === "interactivediagram" || typeNorm === "hotspot" || typeNorm === "labeldiagram") {
    const profile = resolveSubTopicProfile({
      topic: ctx.topic,
      topicKey: ctx.topicKey,
      subTopic: ctx.subTopic,
    });
    let topicKind = resolveInteractiveDiagramTopicKind({
      topic: ctx.topic,
      topicKey: ctx.topicKey,
      subTopic: ctx.subTopic,
    });
    if (isTopicKindBlockedForProfile(topicKind, profile)) {
      topicKind = "generic";
    }
    const specializedKinds = new Set(["brain", "reflexArc", "eye", "cell"]);
    const spec = getInteractiveDiagramTopicSpec(topicKind);
    const selectedTemplate =
      specializedKinds.has(topicKind) && spec?.briefHeader
        ? spec.briefHeader
        : "DIAGRAM BRIEF (generic)";
    logInteractiveDiagramBriefResolution({
      topic: ctx.topic,
      topicKey: ctx.topicKey,
      subTopic: ctx.subTopic,
    ...
    if (specializedKinds.has(topicKind)) {
      const diagram = spec?.diagram || diagramPick.diagram;
      if (diagram) {
        return formatInteractiveDiagramTopicBrief(diagram, topicKind);
      }
    }
  }
...
function appendCoverageGuidanceToBrief(brief, block, ctx) {
  const map = ctx.workingCoverageMap || ctx.coverageMap;
  if (!map || !brief) return brief;

  const typeNorm = normalizeBlockType(block);
  const generationKind = typeNorm === "hotspot" ? "hotspot" : "activity";
  const slot = checkCoverageBeforeGeneration(map, { generationKind });
  if (!slot.conceptName) return brief;

  if (ctx.workingCoverageMap && slot.conceptId) {
    recordPlannedExposure(ctx.workingCoverageMap, slot.conceptId, slot.cognitiveSkill);
  }

  return `${brief}\n\nCoverage (Phase 4):\nFocus concept: ${slot.conceptName}\nCognitive skill: ${slot.cognitiveSkill}\n${slot.rationale}`;
}
 ...
 */
function injectDiagramAndActivityBriefs(pages, brain, options = {}) {
  ...
  const ctx = {
    ...
    usedDiagrams,
    coverageMap,
    workingCoverageMap: cloneCoverageMapForPlanning(coverageMap),
    topic: safeStr(options.topic) || safeStr(brain.topic),
    topicKey: safeStr(options.topicKey) || safeStr(brain.topicKey),
    subTopic: safeStr(options.subTopic) || safeStr(brain.subTopic),
    lessonTitle: safeStr(options.lessonTitle),
    pages,
    brain,
  };
  ...
  const newPages = pages.map((page, pageIndex) => {
    ...
    const newBlocks = blocks.map((block, blockIndex) => {
      ...
      const blockCtx = { ...ctx, pageIndex, blockIndex };
      const brief = appendCoverageGuidanceToBrief(buildBriefForBlock(block, blockCtx), block, blockCtx);
      ...
      const interactiveTopicKind =
        typeNorm === "interactivediagram" || typeNorm === "hotspot" || typeNorm === "labeldiagram"
          ...
          : undefined;
      if (interactiveTopicKind && interactiveTopicKind !== "generic" && interactiveTopicKind !== "metabolism") {
        briefKind = `diagram:${interactiveTopicKind}`;
      }
      const sequenceTopicKind =
        typeNorm === "interactivesequence"
          ? resolveInteractiveSequenceTopicKind({
              topic: ctx.topic,
              topicKey: ctx.topicKey,
      ...
      if (
        sequenceTopicKind &&
        sequenceTopicKind !== "generic" &&
        sequenceTopicKind !== "metabolism"
      ) {
        briefKind = `stepByStep:${sequenceTopicKind}`;
      }
      injections.push({
        pageIndex,
        blockIndex,
        ...
        interactiveDiagramTopicKind: interactiveTopicKind,
        interactiveSequenceTopicKind: sequenceTopicKind,
      });
      return injected;
    });
