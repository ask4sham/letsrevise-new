...
const DASHBOARD_OPENING_SLOTS = [
  { key: "objectives", title: "Lesson Objectives", type: "text", role: "lessonObjectives" },
...
function resolveOpeningSlot(block) {
  ...
  let slot = classifyBlockToArchitectureSlot(block);

  if (role === "lessonobjectives" || role === "objectives") return "objectives";
  if (role === "priorknowledge") return "priorKnowledge";
  if (role === "definition") return "definition";
  if (role === "whyitmatters") return "whyItMatters";
  if (role === "coremodel") return "coreModel";
  if (role === "keyexamples") return "keyExamples";
  if (role === "examvocabulary") return "examVocabulary";
  if (role === "hook" || role === "scenario") return "scenario";
  if (role === "corerule" && isDashboardTeacherFirstEnabled()) return "coreModel";
  if (role === "coreteaching") return "coreTeaching";
...
function buildDashboardTeacherFirstPromptSection(ctx = {}) {
  ...
  const lines = [
    ...
    "",
    '1. type "text", title "Lesson Objectives", role "lessonObjectives"',
...
function enforceDashboardTeacherFirstOpening(draft, ctx = {}) {
  ...
  for (const page of draft.pages) {
    ...
    blocks.forEach((block, index) => {
      const slot = resolveOpeningSlot(block);
      if (!DASHBOARD_OPENING_SLOT_KEYS.includes(slot) || bySlot[slot]) return;
      bySlot[slot] = block;
      used.add(index);
    });

    if (!bySlot.coreTeaching) {
      const teachIdx = blocks.findIndex(
        (b, i) =>
          !used.has(i) &&
          String(b.type || "").toLowerCase() === "text" &&
          String(b.role || "").toLowerCase() === "concept"
      );
      if (teachIdx >= 0) {
        bySlot.coreTeaching = blocks[teachIdx];
        used.add(teachIdx);
      }
    }
