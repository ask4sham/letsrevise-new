...
function scoreTeacherFirstOpeningCoverage(input = {}) {
  ...
  earlyBlocks.forEach((block, idx) => {
    ...
    if (firstScenarioBlockIndex < 0 && blockHasScenarioOpening(block)) {
  ...
  if (firstScenarioBlockIndex >= 0) {
    ...
    if (firstScenarioBlockIndex <= 1) {
      openingTooScenarioHeavy = true;
    }
  }

  if (earlyBlocks[0] && blockHasScenarioOpening(earlyBlocks[0])) {
    openingTooScenarioHeavy = true;
  }
  ...
  const openingScorePct = Math.round((passed / checks.length) * 100);
