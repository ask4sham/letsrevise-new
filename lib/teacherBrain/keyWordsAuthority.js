...
function keywordRowsFrameworkRatio(rows = []) {
  if (!rows.length) return 0;
  return rows.filter((r) => isFrameworkMetaTerm(r.term)).length / rows.length;
}

function rowsMatchGenericFallback(rows = []) {
  const genericTerms = genericKeywordFallbackRows("", 10).map((r) => normalizeTermKey(r.term));
  const terms = rows.slice(0, 10).map((r) => normalizeTermKey(r.term));
  return terms.length >= 10 && genericTerms.every((t, i) => terms[i] === t);
}

/**
 * Replace framework/meta keywords or pad to count using profile-aware fallback.
 * @param {object} opts
 * @param {Array<{term:string,def:string}>} [opts.existingRows]
 */
function reconcileKeywordRows({
  existingRows = [],
  topic = "",
  topicKey = "",
  subject = "Biology",
  ...
  const frameworkRatio = keywordRowsFrameworkRatio(existingRows);
  const genericMatch = rowsMatchGenericFallback(existingRows);
  const subjectRows = existingRows.filter((r) => !isFrameworkMetaTerm(r.term));
  const hasStrongSubjectKeywords =
    subjectRows.length >= 8 && frameworkRatio <= 0.3 && !genericMatch;

  if (hasStrongSubjectKeywords) {
    if (existingRows.length >= count) {
      return {
        rows: existingRows.slice(0, count),
        replaced: false,
    ...
    if (!result.usedGenericFallback) {
      const seen = new Set(existingRows.map((r) => normalizeTermKey(r.term)));
      ...
      return {
        rows: merged.slice(0, count),
        replaced: merged.length !== existingRows.length,
        usedGenericFallback: false,
        fixMessage: "Padded Keywords to exactly 10 profile-aware term–definition lines.",
      };
    }
    return {
      rows: existingRows.slice(0, count),
  ...
  const needsReplace =
    existingRows.length < count || frameworkRatio > 0.5 || genericMatch;
  ...
  let fixMessage = "Normalised Keywords to exactly 10 term–definition lines.";
  if (frameworkRatio > 0.5 || genericMatch) {
    fixMessage = "Replaced exam-framework Key Words with profile-aware subject vocabulary.";
...
function profileAwareKeywordFallback({
  ...
  if (needed > 0) {
    ...
    const pool = terms.length ? terms : subjectRows.map((r) => r.term);
    if (profile && pool.length > 0) {
      let idx = 0;
      while (rows.length < count && idx < pool.length * 6) {
