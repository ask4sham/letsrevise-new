...
const { pedagogyPriorityTeachingNote, resolvePedagogyProfile } = require("./structureFunctionPedagogyEngine");
...
function formatConceptPriorityAppendix(priorityProfile) {
  ...
  const pedagogyNote = pedagogyPriorityTeachingNote(
    resolvePedagogyProfile({ taxonomyKey: priorityProfile.taxonomyKey })
  );
  if (pedagogyNote) {
    lines.push("", pedagogyNote);
  }
