  ...
  hotspots: [
    {
      ...
      y: 12,
      label: "Ruler",
      description: "30 cm ruler held vertically above the catching hand.",
    },
    {
      id: "release-point",
      x: 50,
    ...
    },
    {
      ...
      description: "Read the distance (cm) where the ruler was caught.",
    },
    {
      id: "thumb",
      x: 46,
      y: 78,
      label: "Thumb",
      description: "Part of the catching hand used with the forefinger.",
    },
    {
      id: "forefinger",
      x: 54,
...
const REACTION_TIME_RP_PROFILE = {
  ...
  methodSteps: [
    ...
    "Partner holds ruler vertically above thumb and forefinger.",
...
function ensureSpecialistBlockContent(block = {}, slotKey = "", plan = {}) {
  ...
  return {
    ...
    content: contentValid ? block.content : slotContentForPlan(slotKey, plan),
  };
}
...
function slotContentForPlan(slotKey, plan) {
  ...
  switch (slotKey) {
    ...
    case "diagram":
      return "Diagram showing the ruler-drop test setup: Person A's hand positioned below a vertical ruler held by Person B.";
    default:
      return "";
  }
}
...
function buildRequiredPracticalDashboardPromptSection(ctx = {}) {
  ...
  const lines = [
    "",
    "## REQUIRED PRACTICAL MODE V2 (MANDATORY — REPLACES TEACHER-FIRST KNOWLEDGE OPENING)",
    "",
    buildRequiredPracticalReplacementDirective(),
    "",
    buildRequiredPracticalSpecialistBlocksSection(),
...
function formatRequiredPracticalAppendix(input = {}) {
  ...
  const lines = [
    "--- REQUIRED PRACTICAL MODE V2 (Teacher Brain) ---",
    "",
