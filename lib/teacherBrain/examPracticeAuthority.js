...
function extractQ5ExamPracticeSuffix(body = "") {
  const idx = String(body || "").search(/Q5\s*\(\s*6\s*marks?\)/i);
  if (idx < 0) return "";
  return String(body).slice(idx).trim();
}

function extractExamPracticeQ1Q4Body(body = "") {
  const q5Idx = String(body || "").search(/Q5\s*\(\s*6\s*marks?\)/i);
  return q5Idx >= 0 ? String(body).slice(0, q5Idx) : String(body);
}

function examPracticeBodyFromStems(stems = [], q5Suffix = "") {
...
function analyzeExamPracticeBlock(body, authorityProfile) {
  const q1q4 = extractExamPracticeQ1Q4Body(body);
  const driftTerms = findDriftTermsInText(q1q4);
  const blockedKey = authorityProfile ? detectBlockedInteractionKey(q1q4, authorityProfile) : null;
...
function ensureExamPracticeScopeCompliance(
  ...
  for (let idx = spans.length - 1; idx >= 0; idx--) {
    ...
    const spanBody = span.text.split("\n").slice(2).join("\n");
    const q5Suffix = extractQ5ExamPracticeSuffix(spanBody);
    const newBody = examPracticeBodyFromStems(stems, q5Suffix);
