...
function getSs1CanonicalSlots(ctx = {}) {
  const { isRequiredPracticalMode, REQUIRED_PRACTICAL_SS1_CANONICAL_SLOTS } = require("./requiredPracticalMode");
...
function getSs1SlotIndex(slotKey, ctx = {}) {
...
function buildSs1BlockOrderPromptSection(ctx = {}) {
  const topic = ctx.subTopic || ctx.topic || "this topic";
  const { isRequiredPracticalMode, buildRequiredPracticalSs1BlockOrderSection } = require("./requiredPracticalMode");
  if (isRequiredPracticalMode(ctx)) {
    return buildRequiredPracticalSs1BlockOrderSection(ctx);
  }
...
Do NOT put Scenario before Revision Objectives or Prior Knowledge.
Do NOT put Key words anywhere except after Summary.
Do NOT skip or reorder these required paste targets.`;
}
...
function buildSs1FirstBlocksTemplateSection(ctx = {}) {
  ...
  const { isRequiredPracticalMode, buildRequiredPracticalFirstBlocksTemplateSection } = require("./requiredPracticalMode");
  if (isRequiredPracticalMode(ctx)) {
    return buildRequiredPracticalFirstBlocksTemplateSection(ctx);
  }
  if (!isTeacherFirstSs1Enabled()) {
...
- Revision Objectives (Text concept) as block 1 and Prior Knowledge as block 2
...
function buildSs1AntiDuplicationPromptSection(ctx = {}) {
  const { isRequiredPracticalMode, buildRequiredPracticalAntiDuplicationSection } = require("./requiredPracticalMode");
  if (isRequiredPracticalMode(ctx)) {
    return buildRequiredPracticalAntiDuplicationSection();
  }
  if (!isTeacherFirstSs1Enabled()) return "";

  return `--------------------------------
SS1 OUTPUT RULE (ANTI-DUPLICATION — MANDATORY)
--------------------------------

Output ONLY ONE lesson in numbered SS1 block format.

ALLOWED before block 1:
- LESSON OBJECTIVE FIELD (one plain-text sentence)
- SHORT SUMMARY FIELD (one plain-text sentence)
- PAGE 1
- The lesson title line (<strong>Topic – Organisation …</strong>)

FORBIDDEN before block 1:
- A second unnumbered copy of Definition, Core model, Key examples, or Exam vocabulary
- Freeform <h2> teaching sections outside numbered blocks
- A "preview" lesson followed by numbered blocks

All Definition / Why it matters / Core model / Key examples / Exam vocabulary content MUST appear ONLY inside numbered blocks 3–7.
