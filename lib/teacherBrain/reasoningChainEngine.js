...
function buildReasoningChainPromptSection(meta = {}) {
  ...
  const appendix = buildReasoningChainAppendix(profile);
  if (appendix) return appendix;
  const { buildSubjectIntelligenceReasoningFallback } = require("./subjectIntelligenceEngine");
  return buildSubjectIntelligenceReasoningFallback(meta);
}
