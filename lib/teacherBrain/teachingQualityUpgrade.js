...
function buildTeachingQualityUpgradePromptSection(meta = {}) {
  ...
  const grade89 = buildGrade89ChallengePromptSection(meta);
  const coreDiscipline = buildCoreLearningDisciplinePromptSection(meta);
  const subjectIntelligence = buildSubjectIntelligencePromptSection(meta);
  if (
    !reasoning &&
    !examiner &&
    !workedReasoning &&
...
function evaluateTeachingQualityUpgrade(text = "", meta = {}) {
  ...
  const grade89 = scoreGrade89ChallengeCoverage(text, profile);
  const coreDiscipline = scoreCoreLearningDiscipline(text, profile);
  const subjectIntelligence = scoreSubjectIntelligenceCoverage(text, meta);
  ...
  const grade89Enabled = isGrade89ChallengeEnabled();
  const pass =
    reasoning.pass &&
    examiner.pass &&
    gate.pass &&
    ...
    (grade89.skipped || !grade89Enabled || grade89.pass) &&
    (coreDiscipline.skipped || !isCoreLearningDisciplineEnabled() || coreDiscipline.pass);
  ...
  return {
    ...
    grade89ChallengeEnabled: grade89Enabled,
    coreDisciplineEnabled: isCoreLearningDisciplineEnabled(),
    subjectIntelligenceEnabled: isSubjectIntelligenceEnabled(),
...
function computeTeachingQualityScore(evaluation = {}) {
  ...
  if (evaluation.grade89 && !evaluation.grade89.skipped) {
    max += 1;
    if (evaluation.grade89.pass) score += 1;
  }
  if (evaluation.coreDiscipline && !evaluation.coreDiscipline.skipped) {
    max += 1;
    if (evaluation.coreDiscipline.pass) score += 1;
  }
  if (evaluation.subjectIntelligence && !evaluation.subjectIntelligence.skipped) {
    max += 1;
    if (evaluation.subjectIntelligence.pass) score += 1;
  }
  if (evaluation.gate) {
    max += 1;
