...
} = require("./teacherFirstKnowledgeEngine");
const {
  buildTeachingQualityReview,
  isTeachingQualityEnabled,
} = require("./teachingQualityRubric");
const { buildVtiaTelemetry } = require("./visualTaskInteractionAuthority");
...
function buildLessonCoverageReview(input = {}) {
  ...
  const reasoningCoverage =
    isGcseReasoningEngineEnabled() && reasoningProfile
      ? scoreReasoningCoverage({
          reasoningProfile,
          pages: input.pages,
          topicKey: normalized.topicKey,
  ...
  const interactionAuthority =
    boundaryContext?.active
      ? auditInteractionAuthorityFromLesson({
          pages: input.pages,
          authority: resolveAuthorizedInteractions({
  ...
  const conceptCompressionCoverage =
    isConceptCompressionEnabled() && compressionProfile
      ? scoreConceptCompressionCoverage({
  ...
  const teacherFirstOpeningPlan = buildTeacherFirstOpeningPlan({
    topic: normalized.topic,
    topicKey: normalized.topicKey,
    subTopic: normalized.subTopic,
    subject: normalized.subject,
  });
  const teacherFirstOpeningCoverage = isTeacherFirstOpeningEnabled()
    ? scoreTeacherFirstOpeningCoverage({
        plan: teacherFirstOpeningPlan,
        pages: input.pages,
        topic: normalized.topic,
        topicKey: normalized.topicKey,
        subTopic: normalized.subTopic,
  ...
  const teachingQualityReview = isTeachingQualityEnabled()
    ? buildTeachingQualityReview({
        topic: normalized.topic,
        subTopic: normalized.subTopic,
        topicKey: normalized.topicKey,
  ...
  return {
    ...
    conceptCompressionCoverage,
    teacherFirstOpeningCoverage,
