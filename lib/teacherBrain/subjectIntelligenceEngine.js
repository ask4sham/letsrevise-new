...
const { formatMethodologyAppendix } = require("./archetypeMethodology");
...
function buildSubjectIntelligenceCoreSection(resolved) {
  ...
  const methodology = formatMethodologyAppendix(archetype);
  if (methodology) {
...
function buildSubjectIntelligenceReasoningFallback(meta = {}) {
  ...
  const resolved = resolveSubjectIntelligence(meta);
  const { archetype, subjectKey } = resolved;
  ...
  const coreTeachingHint = isMaths
    ? "Method steps with every line of working visible."
    : "Explain how… with mechanism → outcome.";
  ...
  const lines = [
    ...
    `- Block ${coreTeachingBlock}: ${coreTeachingHint}`,
...
function buildSubjectIntelligenceGrade89Fallback(meta = {}) {
  ...
  const methodology = formatMethodologyAppendix(resolved.archetype);
  if (methodology) {
    lines.push("", methodology);
  }
