...
/** Prefer checkpoint workedExample — saved lessons often duplicate with a weaker text block first. */
function findWorkedExampleBlock(blocks) {
  return (
    blocks.find(
      (b) =>
        String(b?.type || "") === "checkpoint" && String(b?.role || "") === "workedExample"
    ) || blocks.find((b) => String(b?.role || "") === "workedExample")
  );
}

function workedExampleAnswerText(block) {
  if (!block) return "";
  return String(
    block.explanation || block.answer || block.correctAnswer || block.content || ""
  );
}
...
function scoreWorkedReasoning(blocks) {
  ...
  const worked = findWorkedExampleBlock(blocks);
  if (worked) {
    score += 1;
  ...
  const answerText = workedExampleAnswerText(worked);
  if (answerText.length >= 60 && /because|therefore|so that|this means that|consequently/i.test(answerText)) {
    score += 1;
