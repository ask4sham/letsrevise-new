 ...
 * @param {boolean} [opts.semanticFingerprintDedup=true] — when false, only exact embedded IDs are excluded
 */
function filterDistinctPracticeExamQuestions(candidates, opts = {}) {
  ...
  const semanticFingerprintDedup = opts.semanticFingerprintDedup !== false;
  const limit =
  ...
  for (const row of candidates || []) {
    ...
    if (!text) continue;
    if (semanticFingerprintDedup && overlapsExamFingerprint(text, fingerprints, threshold)) continue;
