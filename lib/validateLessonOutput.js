...
/** Strip tags so markers in `<h3>Weak answer:</h3>` are detected like plain text. */
function lessonTextForAnswerMarkers(text = "") {
  return String(text || "")
...
const SPEC_COVERAGE_STOP = new Set([
  "the",
  "and",
  "for",
  "with",
...
function specRequiredLineText(line) {
...
/**
 * Conservative starter-spec coverage warnings (never hard-fails).
 * @param {{ text?: string; specEntry?: object | null }} params
 * @returns {string[]}
 */
export function validateSpecCoverage({ text = "", specEntry = null } = {}) {
  if (!specEntry || !Array.isArray(specEntry.requiredContent)) return [];
  const hay = String(text || "").toLowerCase().replace(/\s+/g, " ");
  if (!hay.trim()) return [];

  const warnings = [];
  for (const line of specEntry.requiredContent) {
    const lineStr = specRequiredLineText(line);
    const words = String(lineStr || "")
      .toLowerCase()
      .split(/\s+/)
      .filter((w) => w.length > 3 && !SPEC_COVERAGE_STOP.has(w));
    if (!words.length) continue;
    const need = Math.min(2, Math.max(1, Math.ceil(words.length * 0.25)));
    const hits = words.filter((w) => hay.includes(w)).length;
    if (hits < need) {
      const short = lineStr.trim().slice(0, 90);
      warnings.push(
        `Specification coverage (starter): lesson may not mention key ideas for — “${short}${short.length >= 90 ? "…" : ""}”`
      );
    }
  }
  return warnings;
...
export function buildSystemIntelligenceReport(blocks = [], text = "") {
  ...
  const answerMarkerHay = lessonTextForAnswerMarkers(fullText);
  ...
  if (!/\bWeak answer\s*:/i.test(answerMarkerHay)) {
...
export function validateLessonOutput(blocks = [], text = "", options = {}) {
  const opts =
    options === null || options === undefined
      ...
      : typeof options === "object" && !Array.isArray(options)
        ? options
        : {};
  const specEntry = opts.specEntry ?? null;
  const specMatchInfo = opts.specMatchInfo ?? null;
  ...
  if (!/\bBetter answer\s*:/i.test(answerMarkerHay)) {
    warnings.push("Missing at least one Better answer example.");
  }

  if (!/\bFull-mark answer\s*:/i.test(answerMarkerHay)) {
  ...
  validateSpecCoverage({ text: fullText, specEntry }).forEach((w) =>
    warnings.push(w)
  );
