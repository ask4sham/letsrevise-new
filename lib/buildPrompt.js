...
} = require("./teacherBrain/dragDropVisualContract.js");
...
const { isRequiredPracticalMode } = require("./teacherBrain/requiredPracticalMode.js");
...
const { resolveSubTopicProfile } = require("./teacherBrain/subTopicProfiles.js");
const { getSubTopicBoundaryMode } = require("./teacherBrain/subTopicBoundaryGuard.js");
const { classifyTopicFramework } = require("../backend/services/topicFrameworkClassification.js");
const { buildFrameworkRoutingPromptSection } = require("./teacherBrain/frameworkRoutingLayer.js");
...
- Keep explanations precise and analytical.`;
  ...
  const ss1BlockOrderSection = buildSs1BlockOrderPromptSection({
    topic,
    subTopic: subTopic || topic,
    topicKey,
  });
  const rpCtx = { topic, subTopic: subTopic || topic, topicKey };
  const rpMode = isRequiredPracticalMode(rpCtx);
  const ss1FirstBlocksSection = buildSs1FirstBlocksTemplateSection(rpCtx);
  const ss1MandatoryInteractiveSection = buildSs1MandatoryInteractiveBlocksSection();
  const teachingQualityUpgradeSection = rpMode
    ? ""
  ...
  const teacherFirstLayer2Section = isTeacherFirstSs1Enabled()
    ? (() => {
        ...
        if (plan.mode === "requiredPractical") {
          return [formatTeacherFirstOpeningAppendix(plan), buildSs1Layer2MandatoryKeywordsSection(plan)]
            .filter(Boolean)
            .join("\n\n");
        }
        const subTopicProfile = resolveSubTopicProfile({
          topicKey,
          subTopic: subTopic || topic,
          topic,
        });
        const boundaryMode = getSubTopicBoundaryMode();
        return [
          formatTeacherFirstOpeningAppendix(plan),
          buildSs1Layer2MandatoryOpeningSection(plan),
          buildSs1Layer2MandatoryObjectivesSection(subTopicProfile),
          formatObjectiveBoundaryAppendix(subTopicProfile, boundaryMode),
          buildSs1Layer2MandatoryCheckpointSection(subTopicProfile),
          buildSs1Layer2MandatoryExamPracticeSection(subTopicProfile),
          buildSs1Layer2MandatorySummarySection(subTopicProfile),
          buildSs1Layer2MandatoryMemoryRuleSection(subTopicProfile),
          buildSs1Layer2MandatoryKeywordsSection(plan),
        ]
          .filter(Boolean)
          .join("\n\n");
      })()
  ...
  const ss1AntiDuplicationSection = buildSs1AntiDuplicationPromptSection(rpCtx);

  const frameworkRoutingSection = buildFrameworkRoutingPromptSection(
...
Revision Objectives and Prior Knowledge MUST each contain full bullet lists (3–5 specific bullets). The parser depends on these bullets.
...
When a Drag and drop match block uses text-to-image main image mode, the artwork MUST follow this contract (markers only — no printed target rectangles; the app draws drop-zone overlays at runtime):

${formatTextToImageImageDesignRequirements()}

When a Drag and drop match block uses diagram mode with on-image drop zones, the artwork MUST follow this contract:
...
- Do Revision Objectives and Prior Knowledge have full bullet lists?
