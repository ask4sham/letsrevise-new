...
const { runTeacherBrain } = require("../teacherBrain");
...
function buildTeacherBrainPromptAppendix(brain) {
  ...
  for (const a of brain.activityRecommendations || []) {
    lines.push(`- After ${a.afterConcept}: ${a.activityType} (${a.cognitiveLevel}) — ${a.rationale}`);
  }
...
function buildTeacherBrainPromptAppendixFromContext(blueprint = {}, ctx = {}) {
  ...
  try {
    const brain = runTeacherBrain(input);
    return buildTeacherBrainPromptAppendix(brain);
  } catch {
    return "";
  }
}
