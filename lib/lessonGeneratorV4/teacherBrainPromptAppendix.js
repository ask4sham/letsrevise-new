...
const { formatBoundaryReplacementAppendix } = require("../teacherBrain/boundaryReplacementPlanner");
const { formatObjectiveBoundaryAppendix } = require("../teacherBrain/objectiveBoundaryEnforcer");
const { formatConceptPriorityAppendix } = require("../teacherBrain/conceptPriorityEngine");
const { formatStructureFunctionPedagogyAppendix, resolvePedagogyProfile } = require("../teacherBrain/structureFunctionPedagogyEngine");
const { buildReasoningAppendix, resolveReasoningProfile } = require("../teacherBrain/gcseReasoningEngine");
const { formatInteractionAuthorityAppendix, resolveAuthorizedInteractions } = require("../teacherBrain/interactionAuthorityLayer");
const {
  formatConceptCompressionAppendix,
  resolveCompressionProfile,
} = require("../teacherBrain/conceptCompressionEngine");
const {
  buildTeacherFirstOpeningPlan,
  formatTeacherFirstOpeningAppendix,
} = require("../teacherBrain/teacherFirstKnowledgeEngine");
const { formatTeachingQualityAppendix } = require("../teacherBrain/teachingQualityRubric");
const { buildTeachingQualityUpgradePromptSection } = require("../teacherBrain/teachingQualityUpgrade");
...
function buildTeacherBrainPromptAppendix(brain) {
  ...
  const boundaryCtx = buildSubTopicBoundaryContext({
    topic: brain.topic,
    topicKey: brain.topicKey,
    subTopic: brain.subTopic,
  });
  const compressionProfile = resolveCompressionProfile({
    topicKey: brain.topicKey,
    subTopic: brain.subTopic,
    topic: brain.topic,
    subTopicProfile: boundaryCtx.subTopicProfile,
  });
  const teacherFirstSection = formatTeacherFirstOpeningAppendix(
    buildTeacherFirstOpeningPlan({
      topic: brain.topic,
      topicKey: brain.topicKey,
      subTopic: brain.subTopic,
  ...
  if (teacherFirstSection) {
    lines.push("", teacherFirstSection);
  }

  const teachingQualitySection = formatTeachingQualityAppendix({
    topic: brain.topic,
    topicKey: brain.topicKey,
    subTopic: brain.subTopic,
    subject: brain.subject,
  });
  if (teachingQualitySection) {
    lines.push("", teachingQualitySection);
  }
  ...
  if (teachingQualityUpgradeSection) {
    lines.push("", teachingQualityUpgradeSection);
  }

  const compressionSection = formatConceptCompressionAppendix(compressionProfile);
  if (compressionSection) {
    lines.push("", compressionSection);
  }

  const boundarySection = formatSubTopicBoundaryAppendix(boundaryCtx);
  if (boundarySection) {
    lines.push("", boundarySection);
  }

  const objectiveBoundarySection = formatObjectiveBoundaryAppendix(
    boundaryCtx.subTopicProfile,
    boundaryCtx.boundaryMode
  );
  if (objectiveBoundarySection) {
    lines.push("", objectiveBoundarySection);
  }

  const prioritySection = formatConceptPriorityAppendix(boundaryCtx.priorityProfile);
  if (prioritySection) {
  ...
  const interactionAuthoritySection = formatInteractionAuthorityAppendix(
    boundaryCtx.interactionAuthority ||
      resolveAuthorizedInteractions({
        subTopicProfile: boundaryCtx.subTopicProfile,
        topicKey: brain.topicKey,
        subTopic: brain.subTopic,
        boundaryMode: boundaryCtx.boundaryMode,
      })
  );
  if (interactionAuthoritySection) {
    lines.push("", interactionAuthoritySection);
  }

  const replacementSection = formatBoundaryReplacementAppendix(
    brain.boundaryReplacementPlan
  );
  if (replacementSection) {
    lines.push("", replacementSection);
  }
...
function buildTeacherBrainPromptAppendixFromContext(blueprint = {}, ctx = {}) {
  ...
  try {
    ...
    const oneShotPreview = buildOneShotLessonCoveragePlanAppendix({
      topic: input.topic,
    ...
    brain.boundaryReplacementPlan =
      oneShotPreview.gate?.replacementPlan || oneShotPreview.gate?.boundary?.replacementPlan;
