...
function mapBlockToMandatorySlot(block, counters) {
  ...
  if (getMandatoryArchitectureSequence().includes(base)) return base;
...
function slotOrderIndex(slot) {
  const idx = getMandatoryArchitectureSequence().indexOf(slot);
...
function enforceLessonStructure(pages, blueprint = null) {
  const mandatorySequence = getMandatoryArchitectureSequence();
  ...
  for (const slot of mandatorySequence) {
    buckets[slot] = [];
  }
  ...
  original.forEach((block) => {
    ...
    const key = mandatorySequence.includes(slot) ? slot : "unclassified";
    buckets[key].push({
      ...block,
  ...
  const foundationSlots = getFoundationSlots();
