      ...
      const genericErr =
        ...
        (e.data.error === "Unhandled server error" || e.data.error === "Request failed" || e.data.error === "Server error")
          ...
          : typeof e?.data?.error === "string"
            ...
            : "";
      const resolved =
        fromInterceptor || apiMsg || genericErr || "Failed to get answer";
      const isUnavailable =
        resolved === ASK_SHAM_UNAVAILABLE ||
        genericErr === ASK_SHAM_UNAVAILABLE ||
        fromInterceptor === ASK_SHAM_UNAVAILABLE;
      setError(isUnavailable ? ASK_SHAM_UNAVAILABLE : resolved);
        setMessages([]);
    } finally {
      setLoading(false);
    }
